...
#Création fichier résultat des scores de classification par concept et par l'histogramme de sift
	./src/probasConceptsColor test/sift_$element.out $element test/res1.txt

        ./src/svm-predict -b 1 test/histoColor.svm train/model/color_$element.model test/color_$element.out
#Création fichier résultat des scores de classification par concept et par l'histogramme de couleurs
	./src/probasConceptsColor test/color_$element.out $element test/res2.txt
	done
...
sort -r -nk1,1 test/resFinal.txt > test/result.txt
